...
DGifGetImageDesc(GifFileType *GifFile)
{
    ...
    BitsPerPixel = (Buf[0] & 0x07) + 1; // Byte 后三位+1为颜色列表的位数
    GifFile->Image.Interlace = (Buf[0] & 0x40) ? true : false;  // Byte 第二位为交织标志
...
/******************************************************************************
 Added by ShaJie
 reset file handle, used for gif images loop: 0 -> n-1 -> 0.
 ******************************************************************************/
int
DGifSetFileOffset(GifFileType *GifFile, long offset)
{
    GifFilePrivateType *Private = (GifFilePrivateType *)GifFile->Private;    
    fseek(Private->File, offset, SEEK_SET);
    return 0;
}

long
DGifGetFileOffset(GifFileType *GifFile)
{
    GifFilePrivateType *Private = (GifFilePrivateType *)GifFile->Private;
    long offset = ftell(Private->File);
    return offset;
}

int
DGifShiftImageDataWithoutDecode(GifFileType * GifFile)
{
    ...
    // 读取 Image Descriptor，9个Byte
    if (DGifGetWord(GifFile, &word) == GIF_ERROR ||
        DGifGetWord(GifFile, &word) == GIF_ERROR ||
        DGifGetWord(GifFile, &word) == GIF_ERROR ||
        DGifGetWord(GifFile, &word) == GIF_ERROR)
    {
        GifFile->Error = D_GIF_ERR_READ_FAILED;
    ...
    Interlace = (Buf[0] & 0x40) ? true : false;  // Byte 第二位为交织标志    // 读取局部颜色列表
    BitsPerPixel = (Buf[0] & 0x07) + 1; // Byte 后三位+1为颜色列表的位数
    if (Buf[0] & 0x80)
    {
        // Note: 这里没有采用直接一次移位的操作，是因为 GifFile 可能是从文件读取，也可能是从 Data 读取，后者的 READ 是给上层传入的，需要记录读取的偏移位置，涉及更多的调整。因此暂时采用跟正常解码一样的读取流程来做移位。
        unsigned int ColorCount = 1 << BitsPerPixel;
        unsigned int i = 0;
    ...
    if (READ(GifFile, &CodeSize, 1) < 1)
    {    /* Read Code size from file. */
        GifFile->Error = D_GIF_ERR_READ_FAILED;
        return GIF_ERROR;    /* Failed to read Code size. */
    }
    
    // 读取图像数据，逐个读取 Data Sub-Blocks，结束处有 block terminator
    GifByteType buffer[256];
    GifByteType size = 0;
    do {
        if (READ(GifFile, &size, 1) != 1)
        {
            GifFile->Error = D_GIF_ERR_READ_FAILED;
            return GIF_ERROR;
        }
        ...
        if (READ(GifFile, buffer, size) != size)
        {
            GifFile->Error = D_GIF_ERR_READ_FAILED;
            return GIF_ERROR;
        }
