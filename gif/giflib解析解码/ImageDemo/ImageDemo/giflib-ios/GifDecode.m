...
void renderGifRowWithRowWidth( PixelRGBA *contextBuffer, GifRowType rowBuffer,
                              GifFileType *gifFile, int rowNum,
                              int transFlag, int transIndex, int rowWidth);
...
void resetRowToBackground( PixelRGBA *contextBuffer, GifFileType *gifFile, int rowNum, int transFlag, int transIndex, int canvasWidth ) {
    int colNum = gifFile->Image.Left;
    int width = gifFile->Image.Width;
    ...
    if ( transFlag != 0 || bkgColorIndex == transIndex) {
        for ( int i = colNum; i < colNum + width; i++ ) {
            int pixIndex = rowNum * canvasWidth + i;
        ...
        GifByteType red = 0, green = 0, blue = 0;
        if (colorMap != NULL && bkgColorIndex < colorMap->ColorCount) {
        ...
        for ( int i = colNum; i < colNum + width; i++ ) {
            int pixIndex = rowNum * canvasWidth + i;
            contextBuffer[pixIndex].red = red;
            contextBuffer[pixIndex].green = green;
            contextBuffer[pixIndex].blue = blue;
            contextBuffer[pixIndex].alpha = 0xff;
        }
    }
}
                  ...
                  int transFlag, int transIndex, int rowWidth)
{
    ...
    for ( int i = colNum; i < colNum + width; i++ ) {
        ...
        if ( transFlag == 0 || pix != transIndex ) {
            GifColorType *colorMapEntry = &colorMap->Colors[pix];
            int pixIndex = rowNum * rowWidth + i;
            contextBuffer[pixIndex].red = colorMapEntry->Red;
            contextBuffer[pixIndex].green = colorMapEntry->Green;
            contextBuffer[pixIndex].blue = colorMapEntry->Blue;
            contextBuffer[pixIndex].alpha = 0xff;
    ...
    if ( gifFile->Image.Interlace ) {
        ...
        for ( int i = 0; i < 4; i++ ) {
            for ( int j = row + InterlacedOffset[i]; j < row + height; j += InterlacedJumps[i] ) {
                if ( DGifGetLine( gifFile, &rowBuffer[col], width ) == GIF_ERROR )
                    return gifFile->Error;
                renderGifRowWithRowWidth( contextBuffer, rowBuffer,
                             gifFile, j, transFlag, transIndex, (int)(bufferSize / (4 * gifFile->SHeight)));
            }
        }
    } else {
        for ( int i = 0; i < height; i++ ) {
            if ( DGifGetLine( gifFile, &rowBuffer[col], width ) == GIF_ERROR )
                return gifFile->Error;
            renderGifRowWithRowWidth( contextBuffer, rowBuffer,
                         gifFile, row, transFlag, transIndex, (int)(bufferSize / (4 * gifFile->SHeight)) );
            row +=1;
        }
    }
    *image = CGBitmapContextCreateImage( context );
    switch ( gcb.DisposalMode ) {
        case DISPOSE_BACKGROUND: {
            int canvasWidth = (int)(bufferSize / (4 * gifFile->SHeight));
            if ( gifFile->Image.Interlace ) {
                for ( int i = 0; i < 4; i++ ) {
                    for ( int j = row + InterlacedOffset[i]; j < row + height; j += InterlacedJumps[i] ) {
                        resetRowToBackground( contextBuffer, gifFile, j, transFlag, transIndex, canvasWidth );
                    }
                }
            } else {
                int row = gifFile->Image.Top;
                for ( int i = 0; i < height; i++ ) {
                    resetRowToBackground( contextBuffer, gifFile, row, transFlag, transIndex, canvasWidth );
                    row +=1;
                }
            }
            break;
            ...
            case EXTENSION_RECORD_TYPE:
            {
                ...
                if ( gifExtCode == GRAPHICS_EXT_FUNC_CODE && gifExtBuffer[0] == 4 ) { // Graphic Control Extension block
                    ...
                    if ( gifInfo ) {
                        ...
                        if ( delayDic == nil ) {
                            delayDic = [[ NSMutableArray alloc ] init ] ;
                ...
                if ( delegate ) {
                    ...
                    if ( separate ) {
                        ...
                        if ( gifInfo ) {
                            ...
                            if ( frameBounds == nil ) {
                                frameBounds = [[ NSMutableArray alloc ] init ];
