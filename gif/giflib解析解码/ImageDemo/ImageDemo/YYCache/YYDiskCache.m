...
- (void)_trimInBackground {
    ...
    static CGFloat trimPercent = 0.8;
    dispatch_async(_queue, ^{
        __strong typeof(_self) self = _self;
        if (!self) return;
        Lock();
        if ([_kv getItemsSize] >= self.costLimit) {
            [self _trimToCost:self.costLimit * trimPercent];
        }
        if ([_kv getItemsCount] >= self.countLimit) {
            [self _trimToCount:self.countLimit * trimPercent];
        }
...
- (instancetype)initWithPath:(NSString *)path {
    return [self initWithPath:path inlineThreshold:0]; // 20KB
}
