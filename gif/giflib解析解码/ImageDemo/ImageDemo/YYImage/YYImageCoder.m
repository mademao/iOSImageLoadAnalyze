...
// When limitedSize is setted, docode image to a smaller size.
// Works for images in PNG, JPEG, WEBP format.
#define YYIMAGE_SG_LIMITSIZE 1
...
CGImageRef YYCGImageCreateDecodedCopyDisplay(CGImageRef imageRef, BOOL decodeForDisplay, NSUInteger displayWidth, NSUInteger displayHeight) {
...
#if YYIMAGE_SG_LIMITSIZE
- (CGSize)limitedSize {
    SEL limitedSizeSel = @selector(limitedSizeValue);
    CGSize limitedSize = CGSizeZero;
    if ([self respondsToSelector:limitedSizeSel]) {
        NSValue *value = (NSValue *)[self performSelector:limitedSizeSel];
        if (value) {
            limitedSize = [value CGSizeValue];
        }
    }
    return limitedSize;
}
...
- (CGSize)displaySize {
    SEL aSel = @selector(displaySizeValue);
    CGSize displaySize = CGSizeZero;
    if ([self respondsToSelector:aSel]) {
        NSValue *value = (NSValue *)[self performSelector:aSel];
        if (value) {
            displaySize = [value CGSizeValue];
        }
    }
    return displaySize;
}
...
- (void)setDisplaySize:(CGSize)displaySize {
    SEL aSel = @selector(setDisplaySizeValue:);
    if ([self respondsToSelector:aSel]) {
        [self performSelector:aSel withObject:[NSValue valueWithCGSize:displaySize]];
    }
}

- (CGFloat)resizeRatio {
    SEL aSel = @selector(resizeRatioValue);
    CGFloat ratio = 1.0;
    if ([self respondsToSelector:aSel]) {
        NSNumber *num = (NSNumber *)[self performSelector:aSel];
        ratio = [num floatValue];
    }
    return ratio;
}

- (void)setResizeRatio:(CGFloat)ratio {
    SEL aSel = @selector(setResizeRatioValue:);
    if ([self respondsToSelector:aSel]) {
        [self performSelector:aSel withObject:[NSNumber numberWithFloat:ratio]];
    }
}

#pragma clang diagnostic pop
- (void)adjustSizeIntoLimitedSize:(CGSize)oriSize {
    CGSize newSize = oriSize;
    CGSize limitedSize = [self limitedSize];
    if ((limitedSize.width > 0 && limitedSize.height > 0)
        && (oriSize.width > 0 && oriSize.height > 0)
        && (oriSize.width > limitedSize.width || oriSize.height > limitedSize.height)) {
        CGFloat widthRatio = limitedSize.width / oriSize.width;
        CGFloat heightRatio = limitedSize.height / oriSize.height;
        CGFloat minRatio = MIN(widthRatio, heightRatio);
        if (minRatio < 1.0) {
            newSize.width = minRatio * oriSize.width;
            newSize.height = minRatio * oriSize.height;
            [self setResizeRatio:minRatio];
            [self setDisplaySize:newSize];
        }
    } else {
        [self setResizeRatio:1.0];
    }
}
#endif
...
#if YYIMAGE_SG_LIMITSIZE
            CGImageRef imageRefDecoded;
            if ([self resizeRatio] < 1.0) {
                imageRefDecoded = YYCGImageCreateDecodedCopyDisplay(imageRef, YES, [self displaySize].width, [self displaySize].height);
            } else {
...
#if YYIMAGE_SG_LIMITSIZE
    [self adjustSizeIntoLimitedSize:CGSizeMake(canvasWidth, canvasHeight)];
    CGFloat resizeRatio = [self resizeRatio];
...
#if YYIMAGE_SG_LIMITSIZE
            if (resizeRatio < 1.0) {
                frame.width = iter.width * resizeRatio;;
                frame.height = iter.height * resizeRatio;
                frame.offsetX = iter.x_offset * resizeRatio;
                frame.offsetY = displaySize.height - iter.y_offset * resizeRatio - frame.height;
            }
#endif
...
#if YYIMAGE_SG_LIMITSIZE
    if (resizeRatio < 1.0) {
        _width = (NSUInteger)displaySize.width;
        _height = (NSUInteger)displaySize.height;
    }
#endif
...
#if YYIMAGE_SG_LIMITSIZE
                [self adjustSizeIntoLimitedSize:CGSizeMake(width, height)];
#endif
                _width = width;
                _height = height;
...
- (CGImageRef)_newUnblendedImageAtIndex:(NSUInteger)index
...
#if YYIMAGE_SG_LIMITSIZE
                CGImageRef imageRefExtended;
                if ([self resizeRatio] < 1.0) {
                    imageRefExtended = YYCGImageCreateDecodedCopyDisplay(imageRef, YES, [self displaySize].width, [self displaySize].height);
                } else {
...
#if YYIMAGE_SG_LIMITSIZE
                NSUInteger displayWidth = ([self resizeRatio] < 1.0) ? [self displaySize].width : _width;
                NSUInteger displayHeight = ([self resizeRatio] < 1.0) ? [self displaySize].height : _height;
                CGContextRef context = CGBitmapContextCreate(NULL, displayWidth, displayHeight, 8, 0, YYCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
                if (context) {
                    CGContextDrawImage(context, CGRectMake(0, roundf(_height * [self resizeRatio] - displayHeight), displayWidth, displayHeight), imageRef);
    ...
    if (_webpSource) {
        ...
        WebPDemuxReleaseIterator(&iter);
#if YYIMAGE_SG_LIMITSIZE
        int offsetX = ([self resizeRatio] < 1.0) ? (int)frame.offsetX : iter.x_offset;
        int offsetY = ([self resizeRatio] < 1.0) ? (int)frame.offsetY : iter.y_offset;
#else
        int offsetX = iter.x_offset;
        int offsetY = iter.y_offset;
#endif
        
        if (extendToCanvas && (offsetX != 0 || offsetY != 0)) {
            void *tmp = calloc(1, length);
            if (tmp) {
                vImage_Buffer src = {pixels, height, width, bytesPerRow};
                vImage_Buffer dest = {tmp, height, width, bytesPerRow};
                vImage_CGAffineTransform transform = {1, 0, 0, 1, offsetX, -offsetY};
                uint8_t backColor[4] = {0};
